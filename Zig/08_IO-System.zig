...
// `std.fmt.fmtSliceHexLower` and `std.fmt.fmtSliceHexUpper` provide hex formatting for strings
// as well as `{x}` and `{X}` for ints.
...
// `std.fmt.fmtIntSizeDec` and `std.fmt.fmtIntSizeBin` output memory sizes in metric (1000) and 
// power-of-two (1024) based notation.
test "B Bi" {
